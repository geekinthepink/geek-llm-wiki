...
When workflows are disabled, the bundled workflow commands are unavailable, the `ultracode` keyword no longer triggers workflow creation, and `/effort ultracode` falls back to `xhigh`.

## Related

* [Subagents](/en/sub-agents) — delegate tasks to isolated Claude sessions
* [Skills](/en/skills) — domain-specific instruction sets
* [Agent teams](/en/agent-teams) — persistent multi-agent collaboration
* [Tools reference](/en/tools-reference) — available tools and their behavior
